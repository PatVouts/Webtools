...
{
    public partial class FrmEvaporatorOptionsPricingIncrease : Form
    {
        public FrmEvaporatorOptionsPricingIncrease()
        {
            InitializeComponent();
        }

        private void frmEvaporatorOptionsPricingIncrease_Load(object sender, EventArgs e)
        {
        ...
        private void LoadOptions()
        {
            ...
            DataTable dtOptions = Query.Select("SELECT * FROM EvaporatorOptions ORDER BY GroupName, Description");
        ...
        private void UpdatePrices(string strReason)
        {
            ...
            for (int i = 0; i < lvReview.Items.Count; i++)
            {
                ...
                tsql += " UPDATE EvaporatorOptions SET Price = " + Convert.ToDecimal(lvReview.Items[i].SubItems[2].Text) + " WHERE GroupName = '" + lvReview.Items[i].SubItems[4].Text.Split(new[] { "|||" } , StringSplitOptions.None)[0] + "' AND Description = '" + lvReview.Items[i].SubItems[4].Text.Split(new[] { "|||" } , StringSplitOptions.None)[1] + "'";
        ...
        private decimal GetCalculatedFactor(int listRowIndex)
        {
            ...
            {
                Public.PushLog(ex.ToString(),"frmEvaporatorOptionsPricingIncrease GetCalculatedFactor");
                return 0m;
            }
        }
