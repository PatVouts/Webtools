...
{
    public partial class FrmGravityCoilBlygoldPricingIncrease : Form
    {
        public FrmGravityCoilBlygoldPricingIncrease()
        {
            InitializeComponent();
        }
        ...
        private void LoadModels()
        {
            ...
            DataTable dtModels = Query.Select("SELECT * FROM GravityCoil ORDER BY REFPLUS_Model");
            ...
            for (int i = 0; i < dtModels.Rows.Count; i++)
            {
                ...
                myItem.SubItems[1].Text = Convert.ToDecimal(dtModels.Rows[i]["BlygoldCoating"]).ToString("N2");
        ...
        private void UpdatePrices(string strReason)
        {
            ...
            for (int i = 0; i < lvReview.Items.Count; i++)
            {
                ...
                tsql += " UPDATE GravityCoil SET BlygoldCoating = " + Convert.ToDecimal(lvReview.Items[i].SubItems[2].Text) + " WHERE REFPLUS_Model = '" + lvReview.Items[i].SubItems[0].Text + "'";
