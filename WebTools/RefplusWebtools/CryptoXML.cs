...
namespace RefplusWebtools
{
    ...
    public class XMLEncryptor
    {
        ...
        private void GenerateKey()
        {
            ...
            var hash = new StringBuilder(Username + Password);
        ...
        {
            MD5 md5 = new MD5CryptoServiceProvider();
            const string hash = Password + Username;
        ...
        public DataSet ReadEncryptedXML(string fileName)
        {
            ...
            {
                Public.PushLog(ex.ToString(),"CryptoXML ReadEncryptedXML catch1");
                Trace.WriteLine(ex.Message + "Cannot perform decryption");
                return null;
            }
            ...
            var rijn = new RijndaelManaged {Padding = PaddingMode.Zeros};
            ICryptoTransform decryptor = rijn.CreateDecryptor(_md5Key, _md5Iv);
            // Allocate byte array buffer to read only the xml part of the file (ie everything following the signature).
            var encryptedXmlData = new byte[(int)fi.Length - _signature.Length];
            ...
            var encryptedXmlStream = new MemoryStream(encryptedXmlData);
            // Create a CryptoStream, bound to the MemoryStream containing the encrypted xml data
            var csDecrypt = new CryptoStream(encryptedXmlStream, decryptor, CryptoStreamMode.Read);
            ...
            {
                Public.PushLog(ex.ToString(),"CryptoXML ReadEncryptedXML catch2");
        ...
        public void WriteEncryptedXML(DataSet dataset, string encFileName)
        {
            ...
            var fOut = new FileStream(encFileName, FileMode.Create);
            WriteSignature(fOut);

            #region Encryption objects
            var rijn = new RijndaelManaged {Padding = PaddingMode.Zeros};
            ICryptoTransform encryptor = rijn.CreateEncryptor(_md5Key, _md5Iv);
            var csEncrypt = new CryptoStream(fOut, encryptor, CryptoStreamMode.Write);
            #endregion
            //Create variables to help with read and write.
            var bin = new byte[BinSize];			// Intermediate storage for the encryption.
            int rdlen = 0;									// The total number of bytes written.
            var totlen = (int)xmlStream.Length;	// The total length of the input stream.

            //Read from the input file, then encrypt and write to the output file.
            while (rdlen < totlen)
            {// The number of bytes to be written at a time.
                int len = xmlStream.Read(bin, 0, bin.Length);											
                if (len == 0 && rdlen == 0)
                {
