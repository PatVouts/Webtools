...
{
    public partial class FrmCondensingUnitFanUpdater : Form
    {
        ...
        public FrmCondensingUnitFanUpdater()
        {
            InitializeComponent();
        }

        private void frmCondensingUnitFanUpdater_Load(object sender, EventArgs e)
        {
        ...
        private void Save()
        {
            if (Public.LanguageQuestionBox("Are you sure you want to update the part to the applicable models ?", "Êtes-vous sûr de vouloir mettre à jour la piece aux modèles applicables?", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                ...
                if (validSelection)
                {
                    ...
                    {
                        string tsql = "UPDATE CondensingUnitCoilLink SET FanModel = '" + cboFan.Text + "' WHERE Model = '" + dtModelsToUpdate.Rows[i]["Model"] + "'";
                        Query.Execute(tsql);
                    }
