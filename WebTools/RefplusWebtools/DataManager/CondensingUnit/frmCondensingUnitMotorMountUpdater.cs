...
{
    public partial class FrmCondensingUnitMotorMountUpdater : Form
    {
        ...
        public FrmCondensingUnitMotorMountUpdater()
        {
            InitializeComponent();
        }

        private void frmCondensingUnitMotorMountUpdater_Load(object sender, EventArgs e)
        {
        ...
        private void Save()
        {
            if (Public.LanguageQuestionBox("Are you sure you want to update the part to the applicable models ?", "Êtes-vous sûr de vouloir mettre à jour la piece aux modèles applicables?", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                ...
                if (validSelection)
                {
                    ...
                    {
                        string tsql = "UPDATE CondensingUnitCoilLink SET MotorMountModel = '" + cboMotorMount.Text + "' WHERE Model = '" + dtModelsToUpdate.Rows[i]["Model"] + "'";
                        Query.Execute(tsql);
                    }
        ...
        private string GetWhereClause(ref bool validSelection)
        {
            ...
            for (int i = 0; i < lvVoltage.Items.Count; i++)
            {
                ...
                {
                    strVoltage += "VoltageID = " + lvVoltage.Items[i].SubItems[2].Text + " OR ";
                }
            }

            if (strVoltage.Length > 0)
            {
                strVoltage = strVoltage.Substring(0, strVoltage.Length - 3);
            }
            else
            {
                validSelection = false;
