...
{
    public partial class FrmCondensingUnitFanGuardUpdater : Form
    {
        ...
        public FrmCondensingUnitFanGuardUpdater()
        {
            InitializeComponent();
        }

        private void frmCondensingUnitFanGuardUpdater_Load(object sender, EventArgs e)
        {
        ...
        private void Save()
        {
            if (Public.LanguageQuestionBox("Are you sure you want to update the part to the applicable models ?", "Êtes-vous sûr de vouloir mettre à jour la piece aux modèles applicables?", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                ...
                if (validSelection)
                {
                    ...
                    {
                        string tsql = "UPDATE CondensingUnitCoilLink SET FanGuardModel = '" + cboFanGuard.Text + "' WHERE Model = '" + dtModelsToUpdate.Rows[i]["Model"] + "'";
                        Query.Execute(tsql);
                    }
