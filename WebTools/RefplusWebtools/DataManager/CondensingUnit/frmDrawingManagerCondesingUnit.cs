...
namespace RefplusWebtools.DataManager.CondensingUnit
{
    public partial class FrmDrawingManagerCondesingUnit : Form
    {
        ...
        private bool ValidateOverwrite()
        {
            ...
            if (dtDrawing.Rows.Count > 0)
            {
                if (Public.LanguageQuestionBox("This drawing logic description already exists. Do you want to overwrite (yes) or cancel (no) ?", "Cette logique de dessin existe déja.  Voulez vous l'écraser (Oui) or annuler (Non) ?", MessageBoxButtons.YesNo) == DialogResult.No)
                {
                    valid = false;
                }
            }
