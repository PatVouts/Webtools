    ...
    class SteamCoil
    {
        ...
        public SteamCoil(string strFinType, decimal decCoilQuantity, decimal decNumberOfRows, decimal decFPI, decimal decFinHeight, decimal decFinLength, decimal decHeaderDiameter, decimal decFinThickness, string strTubeMaterial, decimal decTubeThickness, decimal decCasingThickness, decimal decFinDensity, decimal decFinPricePerLbs, decimal decCasingDensity, decimal decCasingPricePerLbs, decimal decTubeDensity, decimal decTubePricePerLbs, decimal decHeaderQuantity)
        {
            try
            {
                ...
                decimal decReturnBendPrice = GetReturnBendsPrice(strTubeMaterial);
                ...
                decimal decTotalCostForTubeMaterial = GetTotalCostForTubeMaterial(strFinType, strTubeMaterial, decTubeWeight, decTubePricePerLbs);

                decimal decTotalCostForReturnBends = (decFinHeight / _decFaceTube) * decNumberOfRows * decReturnBendPrice;
                ...
                decimal decLaborPressPunching = ((decFinHeight / _decFaceTube) * decNumberOfRows * decFPI * decFinLength) / 2m / _decPressStrokeMin / _decRowWidth / 0.33m;
                ...
                decimal decLaborCoilAssembly = decNumberOfRows * (decFinHeight / _decFaceTube) * decFinLength * 0.0038m;
                ...
                decimal decLaborHeaderCuttingPuncher = 10m * (decHeaderQuantity + Convert.ToDecimal(CoilPriceOption("SecondCopperHeaderQty")));
                ...
                decimal decLaborBrazingSpigotHeadersCoils = (((decFinHeight / _decFaceTube) * 2m) * 0.35m) * 2m;
                ...
                decimal decTotalCorrectedLaborCost = decTotalLaborCost * 2m * Convert.ToDecimal(CoilPriceOption("LaborCorr"));
                ...
                decimal decTotalCostOfTheCoil = (GetSetupValue("COMPLEX") / ((_decCoilQuantityMinRange + _decCoilQuantityMaxRange) / 2m)) + decTotalNetCost;
            ...
            {
                Public.PushLog(ex.ToString(),"SteamCoil SteamCoil");
                _decTotalPrice = 0m;
            }
        }

        //Get Total Cost For Tube Material
        private decimal GetTotalCostForTubeMaterial(string strFinType, string strTubeMaterial, decimal decTubeWeight, decimal decTubePricePerLbs)
        {
            if (strFinType == "P")
            {
                return decTubePricePerLbs * decTubeWeight * 1.5m;
            }
            if (strTubeMaterial == "CU-NI")
            {
                return (decTubePricePerLbs * decTubeWeight) + (GetCopperTubePricePerLbs() * decTubeWeight * 0.75m);
            }
            return decTubePricePerLbs * decTubeWeight * 1.75m;
        }

        private decimal GetCopperTubePricePerLbs()
        {
            return Convert.ToDecimal(Public.DSDatabase.Tables["CoilTubeMaterial"].Select("TubeMaterial ='COPPER'")[0]["PricePerLbs"]);
        }
