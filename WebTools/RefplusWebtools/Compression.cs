    ...
    class Compression
    {
        ...
        public static byte[] Compress(object objMyObject)
        {
            ...
            using (var msObject = new MemoryStream())
            {
                ...
                var zeEntry = new ZipEntry(Guid.NewGuid().ToString()) {DateTime = DateTime.Now};
                ...
                while (true)
                {
                    int bytesRead = msObject.Read(buffer, 0, buffer.Length);
                    if (bytesRead > 0)
        ...
        {
            var sObjectStream = new MemoryStream(bytMyByte);
            ...
            var zInStream = new ZipInputStream(sObjectStream);
